...
test('mint rejects unknown, disabled, and target-mismatched profiles', () => {
  ...
  // repo-read-v1 exists but is not enabled until P6 (vault-v1 opened at P5)
  assert.equal((store.mint({ runId: 'r', profile: 'repo-read-v1', target: 'github:arnaultbretagne/agora' }) as { error: string }).error, 'profile_disabled')
